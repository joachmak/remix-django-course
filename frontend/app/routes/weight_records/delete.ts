...
export const action: ActionFunction = async ({ request }) => {
  ...
  try {
    ...
    return redirect("/hamsters/" + form_data.get("hamster_id")); // Reload current page
  } catch (e) {
    return redirect("/hamsters/" + form_data.get("hamster_id"), 400); // Reload current page with error status
  }
};
